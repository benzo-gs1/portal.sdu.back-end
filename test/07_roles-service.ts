...
import testRole from "@/services/roles/roles/test";
...
describe("Roles Service", function () {
  describe("#authorize()", function () {
    ...
    it("should allow at exact route", () => {
      const isPassing = RolesService.authorize(TEST_ROLE_INDEX, EXACT_ROUTE);
      expect(isPassing).to.be.true;
    });

    it("should allow at any route", () => {
      const isPassing1 = RolesService.authorize(
        TEST_ROLE_INDEX,
        ANY_ROUTE.replace("*", "fetch-1")
      );
      const isPassing2 = RolesService.authorize(
        TEST_ROLE_INDEX,
